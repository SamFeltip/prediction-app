import { pgTable, text, boolean, integer, serial, timestamp, primaryKey, index, uniqueIndex } from 'drizzle-orm/pg-core';
...
export const accounts = pgTable('accounts', {
  ...
  providerAccountIdIdx: uniqueIndex('idx_accounts_provider_account').on(table.providerId, table.accountId),
...
export const verificationTokens = pgTable('verification_tokens', {
  ...
  identifierTokenIdx: uniqueIndex('idx_verification_tokens_identifier_token').on(table.identifier, table.token),
}));
...
export const bets = pgTable('bets', {
  ...
  userMarketIdx: uniqueIndex('idx_bets_user_market').on(table.userId, table.marketId),
}));
